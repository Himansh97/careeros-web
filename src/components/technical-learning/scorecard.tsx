  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="grid gap-px border border-border bg-border">
        {session.scorecard.questions.map((question, index) => <article key={question.questionId} className="grid gap-4 bg-card p-5 md:grid-cols-[42px_1fr_auto]"><div className="font-mono text-xs text-muted-foreground">{String(index + 1).padStart(2, "0")}</div><div><div className="flex items-center gap-2">{question.passed ? <CheckCircle2 className="size-4 text-[--color-success]" /> : <XCircle className="size-4 text-[--color-warning]" />}<h2 className="font-medium">{question.title}</h2></div><p className="mt-2 text-sm text-muted-foreground">{question.summary}</p>{question.differences.map((item) => <p key={item} className="mt-1 text-xs text-[--color-warning]">{item}</p>)}<details className="mt-3"><summary className="cursor-pointer text-xs font-medium">Review solution and debrief</summary><pre className="mt-2 overflow-x-auto border border-border bg-muted/40 p-3 font-mono text-xs whitespace-pre-wrap">{question.solution}</pre><p className="mt-2 text-sm text-muted-foreground">{question.debrief}</p></details></div><span className="font-mono text-sm tabular-nums">{Math.round(question.score * 100)}</span></article>)}
      </section>
