...
export function ResultGrid({ result }: { result: QueryResult | null }) {
  ...
  if (!result.ok) return <p role="alert" className="p-4 text-sm text-[--color-warning]">{result.message ?? "The query did not run."}</p>;
  return (
    <div className="overflow-x-auto" tabIndex={0} aria-label={`Query result, ${result.rowCount} rows`}>
      ...
      {result.truncated && <p className="border-t border-border px-3 py-2 text-xs text-[--color-warning]">Output capped for safety. A truncated result cannot pass a complete-result drill.</p>}
    </div>
  );
}
